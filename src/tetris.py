...
class TetrisEnv(gym.Env):
    ...
    def clear_lines(self):
        ...
        if not num_cleared:
            return 0
        self.reward += num_cleared # reward equal number of cleared lines
    ...
    def step(self, action):
        ...
        if self.tetrimino.is_dropped_for(self.board):
            ...
            if np.any(self.board[:, 0]):  # Indicates the player is dead.
                return self.get_state(), self.reward, True, {'droped': True}
            self.set_piece(self.board)
            self.clear_lines()
            self.choose_random_shape()
        return self.get_state(), self.reward, False, {'droped': droped}
